...
function get_time_mem_stats(r::OneClassActiveLearning.Result)
    if !haskey(r.al_history, :time_fit) || !haskey(r.al_history, :mem_fit) ||
       !haskey(r.al_history, :time_qs) || !haskey(r.al_history, :mem_qs)
        return (total_run_time=missing,
                time_fit_min=missing, time_fit_mean=missing, time_fit_max=missing,
...
function get_al_summary(r::OneClassActiveLearning.Result, metric::Symbol, metric_short_name::String, n=5, stability_window=10)
    if !haskey(r.al_summary, metric)
        ...
        ramp_up = if length(cur_data[:ramp_up]) >= n
                        cur_data[:ramp_up][n]
                  else missing end
...
function calc_stats(r::OneClassActiveLearning.Result)
    ...
    qs = begin
        ...
        qs_submethod = get(r.experiment[:query_strategy][:param], :qss_type, missing)
        qs_eps = get(r.experiment[:query_strategy][:param], :eps, missing)
        qs_lambda = get(r.experiment[:query_strategy][:param], :lambda, missing)
        qs_use_penalty = get(r.experiment[:query_strategy][:param], :use_penalty, missing)
        qs_boundary_shift_agg_func = get(r.experiment[:query_strategy][:param], :boundary_shift_agg_func, missing)
        qs_optimizer = begin
            if haskey(r.experiment[:query_strategy][:param], :optimizer)
                qs_opt_method = haskey(r.experiment[:query_strategy][:param][:optimizer], :swarmsize) ? "ParticleSwarmOptimization" : "BlackBoxOptimization"
                qs_opt_eps = r.experiment[:query_strategy][:param][:optimizer][:eps]
                if qs_opt_method == "ParticleSwarmOptimization"
                    qs_opt_submethod = missing
                    qs_opt_maxiter = r.experiment[:query_strategy][:param][:optimizer][:maxiter]
                    qs_opt_minstep = r.experiment[:query_strategy][:param][:optimizer][:minstep]
                    qs_opt_minfunc = r.experiment[:query_strategy][:param][:optimizer][:minfunc]
                    qs_opt_swarmsize = r.experiment[:query_strategy][:param][:optimizer][:swarmsize]
                else
                    qs_opt_submethod = r.experiment[:query_strategy][:param][:optimizer][:method]
                    qs_opt_maxiter = missing
                    qs_opt_minstep = missing
                    qs_opt_minfunc = missing
                    qs_opt_swarmsize = missing
                end
                (qs_opt_method=qs_opt_method, qs_opt_submethod=qs_opt_submethod, qs_opt_eps=qs_opt_eps, qs_opt_maxiter=qs_opt_maxiter, qs_opt_minstep=qs_opt_minstep, qs_opt_minfunc=qs_opt_minfunc, qs_opt_swarmsize=qs_opt_swarmsize)
            else
                (qs_opt_method=missing, qs_opt_submethod=missing, qs_opt_eps=missing, qs_opt_maxiter=missing, qs_opt_minstep=missing, qs_opt_minfunc=missing, qs_opt_swarmsize=missing)
            end
        end
        (qs=qs, qs_submethod=qs_submethod, qs_eps=qs_eps, qs_lambda=qs_lambda, qs_use_penalty=qs_use_penalty,
         qs_boundary_shift_agg_func=qs_boundary_shift_agg_func, qs_optimizer...)
    end
    ...
    res = (id=id, file_name=file_name, data_set=data_set, split_strategy=split_strategy, initial_pool_strategy=initial_pool_strategy,
            ...
            qs...,
            num_points=num_points, num_dimensions=num_dimensions, num_al_iterations=num_al_iterations, oracle=oracle,
            exit_code=exit_code)
    stats = reduce(merge, [get_time_mem_stats(r),
            get_al_summary(r, :f1_score, "f1"),
